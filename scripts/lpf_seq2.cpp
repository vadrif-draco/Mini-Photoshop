...
void apply_maskv(long long(&offsets)[9], float(&mask)[9], long long(&range)[2], long long(&rowsize), char* (&processed), char* (&original)) {

    for (long long i = range[0]; i < range[1]; i += rowsize) {
        for (long long k = i; k < i + 3; k++) {
            processed[k] = round(float(
